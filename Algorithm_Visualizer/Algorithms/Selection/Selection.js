/**
 * SELECT: This class handles operations necessary to perform the selection sort algorithm.
 * Inside Insert (), Sort () -> Button, SelectionSort (), Swap (), Sleep (), and Display () are
...
class Select {
     ...
     * selectionSort () which will sort the data according to the selection sort
     * algorithm.
     */
    Sort() {
        select.selectionSort(values);
    }
    ...
    Step() {
        ...
        select.selectionSort(values, 0, values.length);
    }

    /**
     * SELECTION_SORT: This function handles the actual selection sort algorithm. This
     * function makes a call to Swap () when necessary and otherwise performs the bulk 
     * of the operation.
     * 
     * @param values: The array that is being sorted
     */
    async selectionSort(values) {
    	status = 1;
        for (var i = 0; i < values.length; i++) {
            var min = i;
            states[i] = 1;
            ...
            for (let j = i + 1; j < values.length; j++) {
                if (values[min] > values[j]) {
                    document.getElementById("stat").innerHTML = ("Minimum: " + values[min]);
                    min = j;
                }
            }
            if (min !== i) {
                await this.swap(values, i, min);
